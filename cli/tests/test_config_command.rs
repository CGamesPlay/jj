...
fn test_config_set_file_with_existing_scopes() -> TestResult {
    let mut test_env = TestEnvironment::default();
    let conf_d = test_env.env_root().join("conf.d");
    std::fs::create_dir_all(&conf_d)?;
    let user_config_path = join_paths([test_env.config_path(), &conf_d])?;
    test_env.set_config_path(&user_config_path);

    let work_conf = conf_d.join("work.toml");
    std::fs::write(
        &work_conf,
        indoc! {"
...
fn test_config_file_validation() -> TestResult {
    ...
    // Allows targeting a valid config file via relative path
    let output = work_dir.run_jj([
        "config",
        "set",
        "--file",
    ...
    // loaded layers and can be targeted with --file. This behavior can be
