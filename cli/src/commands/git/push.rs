...
/// `revsets.git-push`. Use positional arguments or `--bookmark`/`--tag` to push
...
pub async fn cmd_git_push(
    ...
    let remote_pos_is_remote = if let Some(remote_pos) = &args.remote_pos {
        let all_remotes = git::get_all_remote_names(workspace_command.repo().store())?;
        all_remotes
            .iter()
            .any(|r| r.as_str() == remote_pos.as_str())
    } else {
        false
    };
    ...
    } else if remote_pos_is_remote {
        // A positional remote always selects exactly one remote (single-remote
        // only); multi-remote/pattern selection stays exclusive to `--remote`.
        StringExpression::exact(args.remote_pos.as_deref().unwrap())
    } else {
    ...
    if args.all {
        ...
        let use_default_revset = args.bookmark.is_empty()
            ...
            && (args.remote_pos.is_none() || remote_pos_is_remote)
            && args.tag.is_empty()
        ...
        for remote in matching_remotes {
            ...
            let mut bookmarks: Vec<String> = args
                .bookmark
                .iter()
